...
model_train = function(data,outdir){
  ...
  cb= crossbasis(data$Hum_max,
                 lag=lag,
                 argvar = list(fun="ns",knots =  quantile(data$Hum_max,c(80)/100,na.rm=T), 
                               Boundary.knots = range(data$Hum_max,na.rm=T)),
  ...
  saveRDS(model,paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns_hummax.rds"))
  return(data)
}
...
for (i in c(10,14,30)){
  ...
  fwrite(k,paste0(outdir,"/Inputdata_win",data$window[1],"_hummax.csv") )
