...
for (a in agelist){
  ...
  lag = 4
  lagnk = 2
  cb =  crossbasis(data$Hum,lag=lag,
...
lm = lm(Deaths~pre_heatdeath,data=allval)
r2= summary(lm)$r.squared
R = R2(allval$Deaths,allval$pre_heatdeath)
RM = rmse(allval$Deaths,allval$pre_heatdeath)
...
g = ggplot(allval)+geom_point(aes(x=Deaths,y=pre_heatdeath,color = Year))+
  ...
  scale_color_manual(values = c("grey30","#a1d7ed"),
                     ...
                     name = "")
outdir = "Figure_0721"
ggsave(paste0(outdir,"/fig_heat_related_mortality_validation_humi.jpg"),g, width=12, height=12, units="cm", scale=1.3)
