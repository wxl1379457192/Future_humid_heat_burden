...
model_train = function(data,outdir){
  modelname = paste0(outdir,"/stratamodel_win14_",data$age[1],"_",max(data$Week),"_ns_hum95th.rds")
...
gc()
datalist = list(indata[which(Week>=29),],indata[which(Week<29)])
for (d in datalist){
    lag <- 4
    lagnk <-2
    outdir = "Model"
