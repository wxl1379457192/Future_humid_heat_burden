...
CESM_list = grep("Hourly_CESM_future_2023", file_names, value = TRUE)
CESM_data = do.call(rbind,lapply(CESM_list,function(c){
  cesm = read.csv(file.path(dir,c),stringsAsFactors = F)
  base = read.csv(file.path("Future_data_CESM",paste0("Hourly_CESM_future_",substring(c,25,41))),stringsAsFactors = F)
  cesm = cesm[complete.cases(cesm),]
  cesm$year = as.numeric(cesm$year)
  
  cy = cesm[which(cesm$year==2023),]
  base = base[which(base$year==2022),]
  colnames(base)[1] = "base"
  cesm = merge(cy[,c(1,2,11,12,13)],base[,c(1,2,11,13)],by=c("NUTS_ID","month","geometry"))
  cesm$delta = cesm$Humidex_mean-cesm$base
  cesm = cesm[,c("NUTS_ID","month","geometry","year","delta")]
  cesm$rcp = strsplit(c,"_")[[1]][5]
  cesm$member = strsplit(strsplit(c,"_")[[1]][6],".csv")[[1]][1]
  print(paste(c,"has been processed."))
  return(cesm)
}))
write.csv(CESM_data,file.path(outdir,"CESM_delta_2023.csv"),row.names = F)
CESM_data = read.csv(file.path(outdir,"CESM_delta_2023.csv"),stringsAsFactors = F)
###############处理hourly数据，按周聚合数据#########
...
CESM_data = CESM_data[complete.cases(CESM_data),]
weekly =lapply(unique(CESM_data$NUTS_ID),function(geo){
  outname = file.path(outdir,paste0("Future_weekly_data_",geo,"_2023.csv"))
  if(file.exists(outname)){
    print(paste(outname,"has exists"))
    p = read.csv(outname,stringsAsFactors = F)
    return(p)
  }else{
    print(geo)
...
write.csv(weekly,file.path(outdir,"Weekly_data_2023.csv"),row.names = F)