...
model_train = function(data,outdir){
  ...
  saveRDS(model,paste0(outdir,"/stratamodel_",data$age[1],"_ns_hum_",min(data$Year),"_",max(data$Year),".rds"))
...
data = data[Year>=2010&Year<=2014,]
k1 = model_train(data,outdir)
gc()
data = subset(indata,indata$age=="65+")
data = data[Year>=2015&Year<=2019,]
k1 = model_train(data,outdir)
gc()
print(unique(data$age))
#############################
data2 = subset(indata,indata$age=="15-65")
data2 = data2[Year>=2010&Year<=2014,]
k2 = model_train(data2,outdir)
gc()
data2 = subset(indata,indata$age=="15-65")
data2 = data2[Year>=2015&Year<=2019,]
k2 = model_train(data2,outdir)
print(unique(data2$age))
##############model for age 0-15#####################
data3 = subset(indata,indata$age=="0-15")
data3 = data3[Year>=2010&Year<=2014,]
k3 = model_train(data3,outdir)
gc()
data3 = subset(indata,indata$age=="0-15")
data3 = data3[Year>=2015&Year<=2019,]
k3 = model_train(data3,outdir)




model_train_tem = function(data,outdir){
  data$age = as.factor(data$age)
  data$gender_group = paste(data$geo,data$Year)
  ...
  model <- glm(death ~ cb + CDD + CDN+ CDA + UDD + UDN+ UDA + ns(Year, df = 4)+ ns(Week, df = 3)+ geo +log(pop),
               family = quasipoisson(link="log"), data = data, na.action="na.exclude")
  data$predictions <-predict(model,newdata = data,type = "response")
  saveRDS(model,paste0(outdir,"/stratamodel_",data$age[1],"_ns_tem_",min(data$Year),"_",max(data$Year),".rds"))
...
data = subset(indata,indata$age=="65+")
data = data[Year>=2010&Year<=2014,]
k1 = model_train_tem(data,outdir)
gc()
data = subset(indata,indata$age=="65+")
data = data[Year>=2015&Year<=2019,]
k1 = model_train_tem(data,outdir)
...
data2 = subset(indata,indata$age=="15-65")
data2 = data2[Year>=2010&Year<=2014,]
k2 = model_train_tem(data2,outdir)
gc()
data2 = subset(indata,indata$age=="15-65")
data2 = data2[Year>=2015&Year<=2019,]
k2 = model_train_tem(data2,outdir)
...
data3 = subset(indata,indata$age=="0-15")
data3 = data3[Year>=2010&Year<=2014,]
k3 = model_train_tem(data3,outdir)
gc()
data3 = subset(indata,indata$age=="0-15")
data3 = data3[Year>=2015&Year<=2019,]
k3 = model_train_tem(data3,outdir)
