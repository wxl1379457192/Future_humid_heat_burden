...
cb= crossbasis(data$Temp_mean,
               ...
               argvar = list(fun="bs",degree=2,
                             knots =  quantile(data$Temp_mean,c(80)/100,na.rm=T)),
...
cb2= crossbasis(data2$Temp_mean,
                lag=lag,
                argvar = list(fun="bs",degree=2,
                              knots = quantile(data2$Temp_mean,c(80)/100,na.rm=T)), #quantile(data2$Humidex_mean,c(50,90)/100,na.rm=T)
                arglag= list(knots = logknots(lag, lagnk)),
...
cb3= crossbasis(data3$Temp_mean,
                lag=lag,
                argvar = list(fun="bs",degree=2,
                              knots = quantile(data$Temp_mean,c(80)/100,na.rm=T)), #quantile(data2$Humidex_mean,c(50,90)/100,na.rm=T)
                arglag= list(knots = logknots(lag, lagnk)),
