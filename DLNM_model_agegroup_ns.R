...
model_train = function(data,outdir){
  ...
  CDD = onebasis(data$CHD_90th,fun="strata",breaks=c(1,4))
  CDN = onebasis(data$CHN_90th,fun="strata",breaks=c(1,4))
  CDA = onebasis(data$CH_90th,fun="strata",breaks=c(1,4))
  UDD = onebasis(data$IHD,fun="strata",breaks=c(1,4))
  UDN = onebasis(data$IHN,fun="strata",breaks=c(1,4))
  UDA = onebasis(data$IHA,fun="strata",breaks=c(1,4))
  ...
  saveRDS(model,paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns.rds"))
  return(data)
}
...
for (i in c(10,14,30)){
  ...
  fwrite(k,paste0(outdir,"/Inputdata_win",data$window[1],".csv") )
}