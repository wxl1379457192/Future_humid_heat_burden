...
SE_knots = function(data,SEnum,outdir){
  ...
  modelname = paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns_hum95th_knots",SEnum,".rds")
...
SE_yeardf = function(data,dfnum,outdir){
  ...
  UDN = onebasis(data$IHN,fun="strata",breaks=c(1,2,4))
  UDA = onebasis(data$IHA,fun="strata",breaks=c(1,2,4))
  modelname = paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns_hum95th_dfyear",dfnum,".rds")
  if (file.exists(modelname)){
    model <- readRDS(modelname)
  }else{
...
SE_weekdf = function(data,weekdf,outdir){
  ...
  UDN = onebasis(data$IHN,fun="strata",breaks=c(1,2,4))
  UDA = onebasis(data$IHA,fun="strata",breaks=c(1,2,4))
  modelname = paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns_hum95th_dfweek",weekdf,".rds")
  if (file.exists(modelname)){
    model <- readRDS(modelname)
  }else{
