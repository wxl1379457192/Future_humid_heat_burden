...
write.csv(file,paste0("Result/Mortality_prediction_history.csv"),row.names = F)
...
g2= ggplot()+
  geom_linerange(data = all_o, aes(ymin =0, ymax=heatdeath, x=Country_name,color = geo),size=1,alpha=0.5)+
  geom_linerange(data = all_old, aes(ymin =0, ymax=heatdeath, x=Country_name,color = geo),size=2)+
...
outdir = "Figure_0721"
ggsave(paste0(outdir,"/fig2d_summerMortality.pdf"),g2, width=22, height=9, units="cm", scale=1)
