...
filelist  =c("Predicted_total_heat_deaths_ssp126_Warming_1.5.csv")
labellist = c("Global warming of 1.5\u00B0C")
# 读取全球国家边界
...
f = ggplot() +
  ...
  theme(panel.background = element_rect(fill = "#f2fcff"),
        ...
        legend.key.size = unit(2, "cm"),  # Adjust legend size
  ...
  facet_wrap(~source)
ggsave(paste0(figout,"/fig3-ssp126.pdf"),f, width=12, height=14, units="cm", scale=2)
ggsave(paste0(figout,"/fig3-ssp126.jpg"),f, width=12, height=14, units="cm", scale=2)

filelist  =c("Predicted_total_heat_deaths_ssp245_Warming_1.5.csv","Predicted_total_heat_deaths_ssp245_Warming_2.csv")
...
f = ggplot() +
  ...
  scale_fill_manual(name = "Projected heat-related excess deaths\n(per million people)",
                    values = c("#4798b3","#7ab8cc","#b9d5de","#d4e7ee","#fcf2da",
                               "#f0d6d0", "#dfaea3", "#b4604e", "#800000", "#430f02"),
                    labels = c("0-25", "26-50", "51-75","76-100","101-200","201-400","401-600",
  ...
  theme(panel.background = element_rect(fill = "#f2fcff"),
        ...
        strip.text = element_text(size = unit(18, "pt"), face = "bold")) +
  facet_wrap(~source)
ggsave(paste0(figout,"/fig3-ssp245.pdf"),f, width=24, height=14, units="cm", scale=2)
ggsave(paste0(figout,"/fig3-ssp245.jpg"),f, width=24, height=14, units="cm", scale=2)



filelist  =c("Predicted_total_heat_deaths_ssp370_Warming_1.5.csv","Predicted_total_heat_deaths_ssp370_Warming_2.csv",
             "Predicted_total_heat_deaths_ssp370_Warming_3.csv")
labellist = c("Global warming of 1.5\u00B0C","Global warming of 2.0\u00B0C","Global warming of 3.0\u00B0C")
# 读取全球国家边界
world <- ne_countries(scale = "medium", returnclass = "sf")
world <- st_transform(world, crs = 3035)
...
f = ggplot() +
  ...
  scale_fill_manual(name = "Projected heat-related excess deaths\n(per million people)",
                    ...
                    labels = c("0-25", "26-50", "51-75","76-100","101-200","201-400","401-600",
                               "601-800","801-1000","1001-1500"),
                    drop = FALSE,guide = guide_legend(title.position = "left", title.hjust = 0.5))+
  theme(panel.background = element_rect(fill = "#f2fcff"),
        panel.grid.major = element_line(color = "grey93"),
...
ggsave(paste0(figout,"/fig3-ssp370.pdf"),f, width=24, height=27, units="cm", scale=2)
ggsave(paste0(figout,"/fig3-ssp370.jpg"),f, width=24, height=27, units="cm", scale=2)
