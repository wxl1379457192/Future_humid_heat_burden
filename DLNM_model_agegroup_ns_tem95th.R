...
model_train = function(data,outdir){
  ...
  data$IHD = data$Heatday_95th_tem-data$CHD_95th_tem
  data$IHN = data$Heatnight_95th_tem-data$CHN_95th_tem
  data$IHA = data$Heatall_95th_tem-data$CH_95th_tem
  ...
  CDD = onebasis(data$CHD_95th_tem,fun="strata",breaks=c(1,2,4))
  ...
  saveRDS(model,paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns_tem95th.rds"))
  return(data)
}
...
for (i in c(14,10,30)){
    ...
    fwrite(k,paste0(outdir,"/Inputdata_win",data$window[1],"_tem95th.csv") )
  }