...
model_train = function(data,outdir){
  ...
  cb= crossbasis(data$Hum_min,
                 lag=lag,
                 argvar = list(fun="ns",knots =  quantile(data$Hum_min,c(80)/100,na.rm=T), 
                               Boundary.knots = range(data$Hum_min,na.rm=T)),
  ...
  saveRDS(model,paste0(outdir,"/stratamodel_win",data$window[1],"_",data$age[1],"_ns_hummin.rds"))
  return(data)
}
...
fwrite(k,paste0(outdir,"/Inputdata_win",data$window[1],"_hummin.csv") )