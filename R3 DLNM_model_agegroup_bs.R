...
da$Humidex_max = pmax(da$Humidex_max_morning,
                      da$Humidex_max_afternoon,
                      da$Humidex_max_night)
...
cordata = data[,c("Humidex_mean","year","week_num","DD_num","DN_num","pop")]
#############################
data2 = subset(da,da$age=="15-65")
data2$death[which(data2$death==0)]=1
data2$age = as.factor(data2$age)
data2$gender_group = paste(data2$geom,data2$year)
cb2= crossbasis(data2$Humidex_mean,
                lag=lag,
                argvar = list(fun="bs",degree=2,
                              knots = quantile(data2$Humidex_mean,c(80)/100,na.rm=T)), #quantile(data2$Humidex_mean,c(50,90)/100,na.rm=T)
                arglag= list(knots = logknots(lag, lagnk)),
...
data3$gender_group = paste(data3$geom,data3$year)
cb3= crossbasis(data3$Humidex_mean,
                lag=lag,
                argvar = list(fun="bs",degree=2,
                              knots = quantile(data3$Humidex_mean,c(80)/100,na.rm=T)), #quantile(data2$Humidex_mean,c(50,90)/100,na.rm=T)
                arglag= list(knots = logknots(lag, lagnk)),
...
outdir = "pooled_model_age_group_dataV2_bs"
dir.create(outdir)
saveRDS(model,paste0(outdir,"/stratamodel_65+.rds"))
saveRDS(model2,paste0(outdir,"/stratamodel_16-65.rds"))
saveRDS(model3,paste0(outdir,"/stratamodel_0-15.rds"))
